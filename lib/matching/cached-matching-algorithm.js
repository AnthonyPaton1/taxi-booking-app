...
export async function matchDriverToBookingsCached(driver, bookings, options = {}) {
  ...
  if (skipCache) {
    console.log('⏩ Cache skipped - running fresh matching algorithm');
    return matchDriverToBookings(driver, bookings);
  }

  // Try to get cached results
  const cached =  getCachedDriverMatches(driver.id);
  ...
  console.log(`❌ Cache MISS: Running matching algorithm for driver ${driver.id}`);
  const matches = matchDriverToBookings(driver, bookings);
  console.log(`🔍 Matcher returned:`, matches, `Length: ${matches?.length}`);
