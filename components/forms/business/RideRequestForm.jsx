...
const handleSubmit = async (e) => {
  ...
  const parsedInitials = formData.initials.split(',').map(i => i.trim().toUpperCase());
  ...
  if (wheelchairUsers > passengerCount) {
   ...
   if (!parsedInitials.length || parsedInitials[0] === "") {
    setStatus("❌ Please enter initials of all passengers for audit purposes.");
    errorRef.current?.focus();
    return;
  }
  ...
  try {
    ...
    const res = await createBusinessBooking({
      ...formData,
      initials: parsedInitials,
      passengerCount,
      wheelchairUsers,
...
<StatusMessage message={status} type={status?.startsWith("❌") ? "error" : "info"} />
      ...
      <form
        ...
        aria-labelledby="business-booking-form"
      >
        <h2
          id="business-booking-form"
...
/>
  ...
  id="initials"
  name="initials"
  placeholder="e.g. JT, PE, SK"
  required
  aria-label="Passenger Initials"
