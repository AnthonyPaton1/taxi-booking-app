//app/components/dashboard/driver/DailyScheduleClient.jsx
"use client";
...
export default function DailyScheduleClient({
  bookings,  // ✅ Changed from instantBookings + advancedBookings
  driverName,
}) {
  ...
  const handleCompleteBooking = async (bookingId) => {  // ✅ No type needed
    ...
    try {
      const res = await fetch(`/api/bookings/${bookingId}/complete`, {  // ✅ Unified endpoint
        method: "POST",
      });
  ...
  const openInMaps = (pickup, dropoff) => {
    const query = `${encodeURIComponent(pickup)} to ${encodeURIComponent(dropoff)}`;
    window.open(`https://www.google.com/maps/dir/?api=1&destination=${query}`, "_blank");
  };

  // ✅ Bookings already sorted and filtered
        ...
        {bookings.length === 0 ? (
          ...
          <div className="space-y-4">
            {bookings.map((booking) => {
              ...
              const hasBid = booking.acceptedBid;
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-600">
                        <div className="flex items-center gap-1">
                          ...
                          {booking.accessibilityProfile?.ambulatoryPassengers || 1} passenger(s)
                        </div>
                        ...
                        {booking.returnTime && (
                          <div className="flex items-center gap-1">
                            ...
                            Round trip - Return {formatTime(booking.returnTime)}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 text-sm bg-gray-50 p-3 rounded">
                        ...
                        <div>
                          ...
                          {booking.createdBy?.business && (
                            <p className="text-gray-600 text-xs">
                              {booking.createdBy.business.name}
                            </p>
                          )}
                          {booking.createdBy && (
                            <p className="text-gray-600 text-xs">
                          ...
                          <div className="flex-1">
                            ...
                            <ul className="text-blue-700 text-xs space-y-1">
                              ...
                              {booking.accessibilityProfile.additionalNeeds && (
                                <li>• {booking.accessibilityProfile.additionalNeeds}</li>
                              )}
                            </ul>
                          </div>
                        </div>
